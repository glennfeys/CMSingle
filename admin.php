...
/**
 * upload_file - function to handle file upload
 *
 * This function checks if a file is uploaded and then handles the file upload.
 *
 * @return void
 */
 ...
 *
 * This function removes the image specified in the $_POST["fileDelete"] parameter.
 *
 * @return void
 */
...
/**
 * Saves the content of the edited file.
 *
...
/**
 * Reloads the current file or sets a new file to be loaded.
 *
 * @return void
 */
function reload(){
    ...
    if (isset($_POST["goto"])) {
        ...
        // Output message to indicate reload.
        echo "reload";
    }
}
...
/**
 * This function replaces the PHP scripts generated by the PHP scripts back to the initial PHP scripts so that the file is the same as the original.
 *
 * @param string $file The file path to get the content from
 * @param string $content The content to replace the PHP scripts with
 * @return string The new content with PHP scripts replaced
 */
 ...
 *
 * @return string The CSRF token, or an empty string if it is not set in the session.
 */
...
function get_safe_path(string $path): string {
    $path = str_replace(array('/', '\\'), DIRECTORY_SEPARATOR, $path); // Replace forward slashes and backslashes with the appropriate directory separator
    $parts = explode(DIRECTORY_SEPARATOR, $path); // Split the path into an array of parts

    $newParts = []; // Initialize an empty array to hold the sanitized parts
    foreach($parts as $part) {
        if($part === "" || $part === ".") // If the part is empty or a single dot, skip it
            continue;

        if($part === "..") // If the part is a double dot, remove the last part from the sanitized parts array
            array_pop($newParts);
        else // Otherwise, add the part to the sanitized parts array
            array_push($newParts, $part);
    }

    return "./" . implode(DIRECTORY_SEPARATOR, $newParts); // Return the sanitized path as a string
}

/**
 * Checks if user is authorized to access admin panel.
 * 
 * @return void
 */
function check_auth() {
...
/**
 * This function returns an array of webpages in the current directory.
 * It excludes files listed in the $excludedFiles array.
 * 
