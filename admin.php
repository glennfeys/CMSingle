 ...
 * Description: This function handles the uploading of files to the server.
 * Parameters: None
 * Return: None
 ******************************************************************************/
...
/**
 * Retrieves a code snippet from a file and displays it.
 *
 * This function removes any image from the snippet file before displaying it.
 *
 * @return void
 */
function get_snippet() {
    // Remove image from snippet file
    if (isset($_POST["snippet_file"])) {
        $file = get_safe_path($_POST["snippet_file"]);
        if(is_file($file)) {
            $snippet = file_get_contents($file);
            // Remove any image from the snippet
...
/**
 * This function saves the edited content to the specified file.
 * 
 * @return void
 */
function save(){
 ...
 *
 * This function sets the current file to the page specified by the user, if any. If the specified page does not exist, the current file is set to an empty string. The content of the current file is then filled and the function echoes "reload".
 *
 * @return void
 */
function reload(){
...
function fillContent($currentFile='') {
    ...
    $additionalContent = '
        ...
        <script>
            ...
            function removeImg() {
                ...
                .then(response => response.text())
                .then(response => {
                    if (response === "successfully deleted file") {
    ...
    // add save button, links and scripts
...
/**
 * This function replaces PHP code generated by scripts back to the initial PHP scripts so that the file is the same as the original.
 *
 * @param string $file The file path from where the content needs to be retrieved
 * @param string $content The content where the PHP code needs to be replaced
 * @return string The new content with replaced PHP code
 */
function fillPhp($file, $content) {
    $fileCont = file_get_contents($file); // Retrieve the content of the file
    preg_match_all('/<\?.*\?>/Us', $fileCont, $matches1); // Find all PHP code using regex

    // Loop through all the matches and replace them in the content
    for ($i=0; $i<count($matches1[0]); $i++) {
        if ($i === 0 && strpos($content, '<!--CMSingle_PHP_BEGIN-->') > strpos($content, '<!--CMSingle_PHP_END-->')) {
            $content = preg_replace('/^.*<!--CMSingle_PHP_END-->/Us', $matches1[0][$i], $content); // Replace the PHP code at the beginning of the content
        } else if ($i === count($matches1[0])-1 && !strpos($content, '<!--CMSingle_PHP_END-->') && strpos($content, '<!--CMSingle_PHP_BEGIN-->')) {
            $content = preg_replace('/<!--CMSingle_PHP_BEGIN-->.*$/Us', $matches1[0][$i], $content); // Replace the PHP code at the end of the content
        } else {
            // Replace the PHP code in the middle of the content
            // TODO: When there are slashes or other special characters in matches1, things can break
            $content = preg_replace('/(^.*)<!--CMSingle_PHP_BEGIN-->.*<!--CMSingle_PHP_END-->/Us', '$1 '.$matches1[0][$i], $content);
        }
    }

    return $content; // Return the new content with replaced PHP code
}
...
/**
 * This function generates a CSRF field that can be used to protect against cross-site request forgery attacks.
 *
 * @return void This function does not return any value.
 */
function csrf_field() {
    // Output the CSRF field as a hidden input field with the name "_token" and the value set to the result of the "csrf_token" function.
 ...
 *
 * This function takes a string $path as input and returns a string that represents a safe path. The function replaces any forward slashes or backslashes in the input path with the DIRECTORY_SEPARATOR constant, which is the correct directory separator for the current operating system. It then splits the path into an array of parts using the DIRECTORY_SEPARATOR as the delimiter. The function then iterates over each part of the path and removes any empty or "." parts. If it encounters a ".." part, it removes the last part added to the newParts array. Finally, the function returns the new path as a string, starting with "./".
 *
...
/**
 * Checks if the user is authorized to access the admin panel.
 * 
 * @return void
 */
function check_auth() {
...
/**
 * Retrieves a list of webpages in the current directory, excluding any files specified in the $excludedFiles array.
 *
 * @global array $excludedFiles An array of filenames to exclude from the list of webpages.
 * @global string $currentFile The name of the current file being accessed.
 * @return array An array of filenames representing the webpages in the current directory.
 */
