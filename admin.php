 ...
 * Description: This function uploads a file to the server and checks if it's an image or not. 
 * Input: None
 * Output: None
 * 
 * @return void
 * 
 * @throws Exception if the file upload is invalid or there is an error uploading the file.
 ******************************************************************************/
...
/**
 * Removes a file from the server if it exists.
 *
 * @return void
 */
...
/**
 * This function retrieves a code snippet from a file specified in the $_POST array and outputs it to the browser.
 *
 * @return void
 */
...
/**
 * Saves the edited content to the specified file.
 * 
...
/**
 * Reloads the current file if the user has specified a page to go to.
 *
 * @return void
 */
function reload(){
...
/**
 * This function replaces the PHP code generated by scripts with the original PHP scripts, so that the file is restored to its original state.
 *
 * @param string $file The path to the file containing the PHP code.
...
function fillPhp($file, $content) {
    ...
    // Find all PHP code generated by scripts.
    preg_match_all('/<\?.*\?>/Us', $fileCont, $matches1);
    
    // Loop through all matches and replace them with the original PHP scripts.
    for ($i=0; $i<count($matches1[0]); $i++) {
        if ($i === 0 && strpos($content, '<!--CMSingle_PHP_BEGIN-->') > strpos($content, '<!--CMSingle_PHP_END-->')) {
            ...
            // Replace the PHP code with the original script.
 ...
 *
 * This function retrieves the CSRF token from the session if it exists, otherwise it returns an empty string.
 *
...
/**
 * This function outputs a hidden input field containing a CSRF token value.
 *
 * @return void
 */
function csrf_field() {
    // Output the input field with the name "_token" and the value of the csrf_token() function.
 ...
 *
 * This function takes a string path as input and returns a safe path by replacing any forward slashes or backslashes with the appropriate directory separator. It also removes any unnecessary parts of the path such as empty strings or "." and ".." references.
 *
 ...
 *
 * This function checks if the CSRF token is correct and if there is a session/correct password.
 * If the CSRF token is incorrect or there is no session/correct password, the function terminates
...
/**
 * Retrieves a list of webpages in the current directory.
 * 
 ...
 * 
 * @return array - An array of webpage file names in the current directory.
 */
