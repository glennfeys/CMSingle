 ...
 * 
 * Notes:
 * - The function checks if the file is uploaded and if it is an image file.
 * - If the file is uploaded and is an image file, it is moved to the UPLOAD_DIR directory.
 * - If the file already exists in the directory, an error message is displayed.
 * - If there is an error uploading the file, an error message is displayed.
 ******************************************************************************/ 
...
/**
 * This function retrieves a code snippet from a file and outputs it to the browser.
 * 
 * @return void
 */
...
/**
 * Saves the edited content to a file.
 *
 ...
 */
function save(){
    // Check if the file and content are set in the POST request
...
/**
 * Reloads the current page or navigates to a specified page if provided.
 *
...
/**
 * This function replaces the PHP code generated by the PHP scripts back to the initial PHP scripts so that the file is the same as the original.
 *
 * @param string $file The file path of the original PHP script
 * @param string $content The content of the modified PHP script
 * @return string The new content with the PHP code replaced
 */
function fillPhp($file, $content) {
    // Get the content of the original PHP script
    $fileCont = file_get_contents($file);

    // Find all PHP code generated by the PHP scripts
    preg_match_all('/<\?.*\?>/Us', $fileCont, $matches1);

    // Replace the PHP code in the modified PHP script
 ...
 *
 * This function outputs a hidden input field that contains a CSRF token value generated by the `csrf_token()` function.
 * This token value can be used to verify that a form submission was made by the same user that requested the form.
 *
 * @return void
 */
function csrf_field() {
...
/**
 * This function takes a user input path and returns a safe path by replacing any forward slashes or backslashes with the appropriate directory separator.
 * It also removes any unnecessary or potentially harmful path segments such as empty strings, current directory references, and parent directory references.
 *
 * @param string $path The user input path to sanitize
...
/**
 * Checks if user is authenticated and authorized to access admin panel.
 *
 * @return void
 * @throws Exception If CSRF token is incorrect or password is invalid.
 */
function check_auth() {
...
/**
 * This function returns an array of webpages found in the current directory.
 * It excludes any files listed in the $excludedFiles global variable.
 * Only files with .php or .html extensions are included.
 * If $currentFile is empty, the index.php or index.html file is placed at the beginning of the array.
 *
 * @return array An array of webpages found in the current directory.
 */
