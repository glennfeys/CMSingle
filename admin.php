...
function save(){
    ...
    if (isset($_POST["file"]) && isset($_POST["content"])) {
        ...
        $content = '<!doctype html><html>'.$_POST["content"].'</html>'; // Get the content from the POST request and wrap it in HTML tags

        $content = fillPhp($file, $content); // Fill the PHP tags in the content with the appropriate values
...
function fillPhp($file, $content) {
    ...
    // Find all PHP code blocks in the file.
    preg_match_all('/<\?.*\?>/Us', $fileCont, $matches1);

    // Replace the PHP code blocks in the content.
    for ($i=0; $i<count($matches1[0]); $i++) {
        if ($i === 0 && strpos($content, '<!--CMSingle_PHP_BEGIN-->') > strpos($content, '<!--CMSingle_PHP_END-->')) {
            // Replace the PHP code block at the beginning of the content.
            ...
            // Replace the PHP code block at the end of the content.
            $content = preg_replace('/<!--CMSingle_PHP_BEGIN-->.*$/Us', $matches1[0][$i], $content);
        } else {
            // Replace the PHP code block in the middle of the content.
            // Note: When there are slashes or other special characters in $matches1, things can break.
...
/**
 * This function outputs a CSRF field.
 *
 * @return void
 */
function csrf_field() {
...
/**
 * This function takes a user input path and returns a safe path by replacing all forward slashes and backslashes with the DIRECTORY_SEPARATOR constant and removing any unnecessary or potentially dangerous parts of the path such as empty strings, dots, and double dots.
 *
...
/**
 * This function checks if the user is authorized to access the admin panel.
 *
 * @return void
 */
function check_auth() {
 ...
 *
 * @global array $excludedFiles An array of files to exclude from the list of webpages.
 * @global string $currentFile The current webpage file.
 * @return array An array of webpage file names.
 */
