...
/**
 * Retrieves a code snippet from a file specified in the POST request and displays it.
 *
 * @return void
 */
 ...
 *
 * This function sets the current file to the page specified by the user, if any. If the specified page does not exist, the current file is set to an empty string. The content of the current file is then filled and the page is reloaded.
 *
 * @global string $currentFile The current file being viewed.
 * @return void
 */
function reload(){
    global $currentFile;
...
/**
 * This function takes a file and its content as input and replaces the PHP scripts generated by the PHP scripts back to the initial PHP scripts so that the file is the same as the original.
 *
...
function fillPhp($file, $content) {
    ...
    // Find all the PHP scripts in the file.
    preg_match_all('/<\?.*\?>/Us', $fileCont, $matches1);

    // Loop through all the PHP scripts found in the file.
    for ($i=0; $i<count($matches1[0]); $i++) {
        // If it is the first PHP script and the content has the end tag before the begin tag, replace the end tag with the PHP script.
        ...
        // If it is the last PHP script and the content has the begin tag but not the end tag, replace the begin tag with the PHP script.
        ...
        // If it is not the first or last PHP script, replace the PHP script between the begin and end tags with the current PHP script.
        } else {
...
/**
 * Output Cross-Site Request Forgery (CSRF) field.
 *
 * This function generates a hidden input field that contains a CSRF token. 
 * This token is used to protect against CSRF attacks by verifying that the 
 * request came from the same origin as the form that generated the token. 
 * This function should be used in all forms that submit data to the server.
 *
 * @return void
 */
function csrf_field() {
 ...
 *
 * This function takes in a string $path and returns a string that is a safe path, 
 * meaning it replaces any forward slashes or backslashes with the DIRECTORY_SEPARATOR constant.
 * It also removes any unnecessary parts of the path such as "." or "..".
 *
 ...
 *
 * This function checks if the CSRF token is correct and if the user has provided the correct password or has an active session.
 * If the user is authorized, the function sets the session password and returns nothing.
 * If the user is not authorized, the function unsets the session password, generates a new CSRF token, deletes the admin content file (if it exists), and displays an HTML form to enter the password.
 *
 * @return void
 */
function check_auth() {
...
/**
 * Returns an array of web page file names in the current directory.
 * Excludes files listed in the global $excludedFiles array.
 * If $currentFile is empty, the index page(s) will be listed first.
 * 
 * @global array $excludedFiles An array of file names to exclude from the list.
 * @global string $currentFile The name of the current file.
 * @return array An array of web page file names.
 */
