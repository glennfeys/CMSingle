 ...
 *
 * This function removes the file specified in the $_POST["fileDelete"] parameter from the server.
 ...
 * 
 * This function retrieves a code snippet from a file specified in the $_POST array and displays it on the page.
 * 
 * @return void
 */
...
/**
 * Executes when 'Save' is pressed and sets the value to the edited value
 *
 * @return void
 */
function save(){
    // Check if file and content are set in $_POST
    if (isset($_POST["file"]) && isset($_POST["content"])) {
        // Get safe path for the file
        $file = get_safe_path($_POST["file"]);

        // Wrap content in HTML tags
        $content = '<!doctype html><html>'.$_POST["content"].'</html>';

        // Fill PHP tags in the content
        $content = fillPhp($file, $content);

        // Remove contenteditable attribute from HTML tags
        $content = str_replace('contenteditable="true"', '', $content);

        // Replace PHP tags with correct syntax
...
/**
 * Reloads the current file or sets a new file to be loaded based on user input.
 *
 * @return void
 */
function reload(){
    ...
    if (isset($_POST["goto"])) {
        ...
        // if the file does not exist, reset the current file
        if(!file_exists($currentFile)) {
            $currentFile = "";
        }

        // fill the content of the current file
        fillContent($currentFile);

        // output a message to indicate that the page has been reloaded
        echo "reload";
    }
}
...
/**
 * This function replaces the PHP code generated by scripts with the original PHP scripts, making the file identical to the original.
 *
 * @param string $file The path to the file containing the original PHP scripts.
 * @param string $content The content of the file with generated PHP code.
 * @return string The new content with the original PHP scripts.
 */
function fillPhp($file, $content) {
    // Get the content of the file with the original PHP scripts.
    $fileCont = file_get_contents($file);
    
    // Find all occurrences of PHP code generated by scripts.
    preg_match_all('/<\?.*\?>/Us', $fileCont, $matches1);

    // Replace the generated PHP code with the original PHP scripts.
    for ($i=0; $i<count($matches1[0]); $i++) {
        if ($i === 0 && strpos($content, '<!--CMSingle_PHP_BEGIN-->') > strpos($content, '<!--CMSingle_PHP_END-->')) {
            ...
            // Replace the generated PHP code with the original PHP scripts.
            // Note: special characters can cause issues.
            $content = preg_replace('/(^.*)<!--CMSingle_PHP_BEGIN-->.*<!--CMSingle_PHP_END-->/Us', '$1 '.$matches1[0][$i], $content);
        }
    }

    // Return the new content with the original PHP scripts.
    return $content;
}
 ...
 *
 * This function retrieves the CSRF token from the session if it exists, and returns it as a string.
 *
 * @return string The CSRF token, or an empty string if it does not exist.
 */
 ...
 * The token is generated using the csrf_token() function.
 *
 * @return void
 */
function csrf_field() {
...
/**
 * This function takes a user input path and returns a safe path.
 *
 * @param string $path The user input path to be sanitized.
 * @return string The sanitized path.
 */
function get_safe_path(string $path): string {
    // Replace forward slashes and backslashes with the platform-specific directory separator.
    $path = str_replace(array('/', '\\'), DIRECTORY_SEPARATOR, $path);

    // Split the path into its individual parts.
    $parts = explode(DIRECTORY_SEPARATOR, $path);

    // Create a new array to hold the sanitized path parts.
    $newParts = [];

    // Loop through each path part.
    foreach($parts as $part) {
        // If the part is empty or a single dot, skip it.
        if($part === "" || $part === ".")
            continue;

        // If the part is a double dot, remove the last part from the new array.
        if($part === "..")
            array_pop($newParts);
        else
            // Otherwise, add the part to the new array.
...
/**
 * Check if user is authenticated and authorized to access admin panel
 *
 * @return void
 */
function check_auth() {
 ...
 * Excludes files specified in the global $excludedFiles array.
 * 
 * @return array An array of webpage filenames.
 */
